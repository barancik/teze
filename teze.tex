...
%Metrics for automatic evaluation of machine translation (MT) are essential not 
%only for measuring the quality of translations and comparing different systems 
%and approaches, but also for the development and tuning of the translation systems 
%themselves.
...
%[Bojar - kniha]  %nizka shoda pro cestinu?!! Ani ne kniha, ale spis wmt13, co 14?, kde to byla
% nejak hrozive nizka shoda..

The first metric correlating well with human judgment was BLEU \cite{bleu} and it 
still remains the most common metric for MT evaluation, even though other, 
...
% sem by se hodil asi opravdovej vzorecek misto  jen zminy
sentence and the corresponding reference sentences, i.e., translations made 
by a human translator. However, the standard practice is using only one 
reference sentence and BLEU then tends to perform badly (see \Fref{example_of_BLEU_malfunction}). 
